...
#define PRESET "8" // CHANGED: SVT-AV1 uses numeric presets (0-13) instead of strings like "medium"
#define CRF "35"   // CHANGED: 35 is a standard starting CRF for AV1 compared to 23 for x264
...
int main(int argc, char **argv) {
  ...
  char *codec_name = "libsvtav1"; // CHANGED: Defaulted the encoder to SVT-AV1
  if (argc >= 3) {
    codec_name = argv[2];
  }
  ...
  long uncompressed_size = (long)(H * W * 1.5 * N); // CHANGED: YUV420P requires 1.5 bytes per pixel, not 3 (which is for RGB24)

  printf("real size       : %ldB\n", uncompressed_size); // CHANGED: Replaced W*H*3*N calculation
  printf("compressed size : %ldB\n", compressed_size);
  printf("spatial ratio   : %.2f\n", (double)uncompressed_size / compressed_size); // CHANGED: Fixed "statial" typo and applied corrected byte size
