...
#include <libavutil/hwcontext.h> // <- Added
...
int main(int argc, char **argv) {
  ...
  if (argc < 3) {
    printf("Usage: ./av1 {input_file} {output_file} [codec_name] [preset] [crf]\n");
    return 1;
  }
  ...
  AVFrame *frame = NULL;
  AVPacket *pkt = NULL;
  AVBufferRef *hw_device_ctx = NULL; // <- Added
  ...
  if (strstr(codec_name, "qsv")) { // <- Added
    if (av_hwdevice_ctx_create(&hw_device_ctx, AV_HWDEVICE_TYPE_QSV, NULL, NULL, 0) == 0) { // <- Added
      fprintf(stderr, "using quick sync!\n");
      codec_ctx->hw_device_ctx = av_buffer_ref(hw_device_ctx); // <- Added
      codec_ctx->pix_fmt = AV_PIX_FMT_QSV; // <- Added
      
      AVBufferRef *hw_frames_ref = av_hwframe_ctx_alloc(hw_device_ctx); // <- Added
      AVHWFramesContext *frames_ctx = (AVHWFramesContext *)(hw_frames_ref->data); // <- Added
      frames_ctx->format    = AV_PIX_FMT_QSV; // <- Added
      frames_ctx->sw_format = AV_PIX_FMT_NV12; // <- Added
      frames_ctx->width     = W; // <- Added
      frames_ctx->height    = H; // <- Added
      frames_ctx->initial_pool_size = 32; // <- Added
      if (av_hwframe_ctx_init(hw_frames_ref) == 0) { // <- Added
        codec_ctx->hw_frames_ctx = av_buffer_ref(hw_frames_ref); // <- Added
      } // <- Added
      av_buffer_unref(&hw_frames_ref); // <- Added
    } else { // <- Added
      fprintf(stderr, "Warning: Failed to initialize QSV hardware.\n"); // <- Added
    } // <- Added
  } else { // <- Added
    codec_ctx->pix_fmt = AV_PIX_FMT_YUV420P; // <- Changed (moved into else block)
  } // <- Added
  ...
  frame->format = hw_device_ctx ? AV_PIX_FMT_NV12 : codec_ctx->pix_fmt; // <- Changed
  ...
  AVFrame *hw_frame = NULL; // <- Added

  if (hw_device_ctx) hw_frame = av_frame_alloc(); // <- Added
    ...
    if (in_pkt->stream_index == video_stream_idx) {
      ...
      while (avcodec_receive_frame(dec_ctx, dec_frame) == 0) {
        ...
        av_image_copy(frame->data, frame->linesize,
                      ...
                      (enum AVPixelFormat)frame->format, codec_ctx->width, codec_ctx->height); // <- Changed

        frame->pts = pts++;
        (void)frame_count;

        if (hw_device_ctx) { // <- Added
          av_hwframe_get_buffer(codec_ctx->hw_frames_ctx, hw_frame, 0); // <- Added
          av_hwframe_transfer_data(hw_frame, frame, 0); // <- Added
          hw_frame->pts = frame->pts; // <- Added
          avcodec_send_frame(codec_ctx, hw_frame); // <- Added
          av_frame_unref(hw_frame); // <- Added
        } else { // <- Added
          avcodec_send_frame(codec_ctx, frame); // <- Added
        } // <- Added
  ...
  if (hw_frame) av_frame_free(&hw_frame); // <- Added
  ...
  if (hw_device_ctx) av_buffer_unref(&hw_device_ctx); // <- Added
