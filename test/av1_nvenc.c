...
int main(int argc, char **argv) {
  ...
  if (argc < 3) {
    printf("Usage: ./encode_nvenc {input_file} {output_file} [codec_name] [preset] [cq]\n");
    return 1;
  }
  ...
  if (avcodec_open2(codec_ctx, enc_codec, &opts) < 0) {
    fprintf(stderr, "Fatal: Could not open the NVENC encoder. Check VRAM and hardware support!\n");
    return 1;
  }
  av_dict_free(&opts);
